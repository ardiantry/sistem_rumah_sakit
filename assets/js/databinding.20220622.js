...
function bindDataMaster($) {
    $.getJSON(BASE_URL + 'RawatJalan/getDataMasterAjax', function(data) {
        ...
        bindCombo({
            $combo: $('#inputGolonganDarah'),
            dataSource: data.golongan_darah,
            dataTextField: 'text',
            dataValueField: 'value'
        });
        bindCombo({
            $combo: $('#formGolonganDarah'),
            dataSource: data.golongan_darah,
            dataTextField: 'text',
            dataValueField: 'value'
        });
            ...
            onSelectedIndexChanged: function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (typeof pendaftaranObj.idDokter !== 'undefined')
    ...
    bindAutoComplete(autoCompleteUnitLayanan);
...
function LayanandBinding($) {
    ...
    $("#tableLayananModal tbody").on('click', 'button', function() {
        ...
        console.log(arr);
        let row_template = `<tr><td class="d-none" data-label="id">${arr.id_layanan}</td><td data-label="jumlah" class="text-center align-middle"><input type="number" class="form-control form-control-sm text-center input-jumlah" value="1" step="1"></td><td data-label="nama" class="text-left align-middle">${arr.nama_layanan}</td><td class="d-none" data-label="harga">${arr.harga_layanan}</td><td class="text-right align-middle" data-label="button-delete"><div class="btn-group btn-group-sm"><button type="button" class="btn btn-danger btn-delete"><i class="fas fa-trash"></i></a></div></td></tr>`;

        let index_exist = -1;
        $table.find('tbody tr').each(function(index, el) {
            let id = $(el).find('td[data-label="id"]').html();
            if (id == arr.id_layanan) {
                index_exist = index;
