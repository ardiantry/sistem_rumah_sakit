...
function handleStep1() {
    ...
    $("#FormPasien").submit(function(event) {
        ...
        if ($("#FormPasien").valid()) {
            ...
            patient.tanggal_lahir = moment($('#dateFormTanggalLahir').datetimepicker('viewDate')).format('YYYY-MM-DD');            
    ...
    $('#formKTP').on('blur', function() {
        ...
        $.when(fetchIhsPasien($(this).val())).done(function (response) {
            if (response.data.entry.length > 0) {
                const data = response.data.entry[0].resource;
                $('#formNamaPasien').val(data.name[0].text);
                $('#formJenisKelamin').val((data.gender == 'male' ? 'L' : 'P'));
                $('#formIHS_ID').val(data.id);
                $('#formStatusIHS').val((data.id) ? 'Terdaftar' : 'Tidak Terdaftar');
            }
            SpinnerOff();
        });
    });
